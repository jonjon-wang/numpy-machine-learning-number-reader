...
def runnetwork(input, dataset, targetoutput):
    ...
    for layer in range(len(n2) - 1, 0, -1):
        if layer == len(n2) - 1:
            ...
            dn2[layer] = (np.matmul(w3[layer + 1].T, (sigmoidtodsigmoid(n2[layer + 1]) * dn2[layer + 1]).T)).T

        db2[layer] =  db2[layer] + sigmoidtodsigmoid(n2[layer]) * dn2[layer]

        dw3[layer] = dw3[layer] + np.matmul((sigmoidtodsigmoid(n2[layer]) * dn2[layer]).T, n2[layer - 1]) #check if this should be n2[layer-1] or not and try to sort through that
